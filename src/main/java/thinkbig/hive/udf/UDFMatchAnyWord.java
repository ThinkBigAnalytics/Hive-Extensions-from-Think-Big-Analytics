...
// use keywords (exact match)
public final class UDFMatchAnyWord extends UDF {  
  public BooleanWritable evaluate(final Text text, final Text pattern) {
    if (text == null || pattern == null) { return null; }
    return new BooleanWritable(MatchUDFUtil.match(text.toString(), pattern.toString(), "keyword", MatchType.SomeWords));
  }
}
